...
export default function FilmaticLines1Page() {
  ...
  const handleViewForm = async (form: FilmaticLinesForm1) => {
    if (form.id) {
      try {
        const res = await filmaticLinesForm1Api.getForm(form.id)
        ...
        setSelectedForm({ ...form, ...nonNullDetail } as FilmaticLinesForm1)
      } catch {
        setSelectedForm(form)
      }
    } else {
                ...
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-light text-blue-600">{latestForm.day_shift_id ? 1 : 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-light text-gray-600">Night Shift Entries</span>
                      <span className="text-xs font-light text-blue-600">{latestForm.night_shift_id ? 1 : 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-light text-gray-600">Total Details Recorded</span>
                      <span className="text-xs font-light text-green-600">
                        {(latestForm.day_shift_id?.shift_details ? 1 : 0) +
                          (latestForm.night_shift_id?.shift_details ? 1 : 0)}
                      </span>
                    </div>
                  </div>
                </div>

...
export default function SterilisedMilkProcessPage() {
  ...
  const pastoTableColumns = [
    { header: "Date",                   key: "date",                            accessorKey: "date",                            getValue: (row: any) => row.date ?? "—",                                                                cell: ({ row }: any) => <span className="text-sm font-light">{row.original.date ?? "—"}</span> },
    { header: "Metric",                 key: "metric",                          accessorKey: "metric",                          getValue: (row: any) => row.metric ?? "—",                                                              cell: ({ row }: any) => <span className="text-sm font-light capitalize">{row.original.metric ?? "—"}</span> },
    { header: "Production Start",       key: "production_start",                accessorKey: "production_start",                getValue: (row: any) => row.production_start ?? "—",                                                    cell: ({ row }: any) => <span className="text-sm font-light">{row.original.production_start ?? "—"}</span> },
    { header: "Production End",         key: "production_end",                  accessorKey: "production_end",                  getValue: (row: any) => row.production_end ? new Date(row.production_end).toLocaleString() : "—",       cell: ({ row }: any) => <span className="text-sm font-light">{row.original.production_end ? new Date(row.original.production_end).toLocaleString() : "—"}</span> },
    { header: "Preheating Start",       key: "preheating_start",                accessorKey: "preheating_start",                getValue: (row: any) => row.preheating_start ?? "—",                                                    cell: ({ row }: any) => <span className="text-sm font-light">{row.original.preheating_start ?? "—"}</span> },
    { header: "Water Circulation",      key: "water_circulation",               accessorKey: "water_circulation",               getValue: (row: any) => row.water_circulation ?? "—",                                                   cell: ({ row }: any) => <span className="text-sm font-light">{row.original.water_circulation ?? "—"}</span> },
    { header: "Temp Hot Water",         key: "temp_hot_water",                  accessorKey: "temp_hot_water",                  getValue: (row: any) => row.temp_hot_water ?? "—",                                                      cell: ({ row }: any) => <span className="text-sm font-light">{row.original.temp_hot_water ?? "—"}</span> },
    { header: "Temp Product Past.",     key: "temp_product_pasteurisation",     accessorKey: "temp_product_pasteurisation",     getValue: (row: any) => row.temp_product_pasteurisation ?? "—",                                         cell: ({ row }: any) => <span className="text-sm font-light">{row.original.temp_product_pasteurisation ?? "—"}</span> },
    { header: "Homo. P Stage 1",        key: "homogenisation_pressure_stage_1", accessorKey: "homogenisation_pressure_stage_1", getValue: (row: any) => row.homogenisation_pressure_stage_1 ?? "—",                                    cell: ({ row }: any) => <span className="text-sm font-light">{row.original.homogenisation_pressure_stage_1 ?? "—"}</span> },
    { header: "Homo. P Stage 2",        key: "homogenisation_pressure_stage_2", accessorKey: "homogenisation_pressure_stage_2", getValue: (row: any) => row.homogenisation_pressure_stage_2 ?? "—",                                    cell: ({ row }: any) => <span className="text-sm font-light">{row.original.homogenisation_pressure_stage_2 ?? "—"}</span> },
    { header: "Total Homo. P",          key: "total_homogenisation_pressure",   accessorKey: "total_homogenisation_pressure",   getValue: (row: any) => row.total_homogenisation_pressure ?? "—",                                       cell: ({ row }: any) => <span className="text-sm font-light">{row.original.total_homogenisation_pressure ?? "—"}</span> },
    { header: "Temp Product Out",       key: "temp_product_out",                accessorKey: "temp_product_out",                getValue: (row: any) => row.temp_product_out ?? "—",                                                    cell: ({ row }: any) => <span className="text-sm font-light">{row.original.temp_product_out ?? "—"}</span> },
  ]
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {viewMode === "table" && (
              <LoadingButton
                onClick={() => exportToExcel(pastoTableColumns.map(c => ({ header: c.header, key: c.key, getValue: c.getValue })), pastoTableData, "steri-pasto-table")}

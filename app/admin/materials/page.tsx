...
export default function MaterialsPage() {
  ...
  // Filter raw materials client-side
  const filteredRawMaterials = useMemo(() => {
    return (rawMaterials || []).filter(rawMaterial => {
      // Search filter
      if (tableFilters.search) {
        const search = tableFilters.search.toLowerCase()
        const name = (rawMaterial.name || "").toLowerCase()
        const desc = (rawMaterial.description || "").toLowerCase()
        if (!name.includes(search) && !desc.includes(search)) return false
      }
      // Name filter
      if (tableFilters.name && !rawMaterial.name?.toLowerCase().includes(tableFilters.name.toLowerCase())) return false
      // Description filter
      if (tableFilters.description && !rawMaterial.description?.toLowerCase().includes(tableFilters.description.toLowerCase())) return false

      return true
    })
  }, [rawMaterials, tableFilters])

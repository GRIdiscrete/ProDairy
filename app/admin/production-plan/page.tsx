  ...
  const filteredPlans = useMemo(() => {
    return (productionPlans || []).filter(plan => {
      ...
      if (tableFilters.search) {
        ...
        if (!plan.name.toLowerCase().includes(search)) return false
      }
      // Status filter
      if (tableFilters.status && plan.status !== tableFilters.status) return false
      // Supervisor filter
      if (tableFilters.supervisor && plan.supervisor !== tableFilters.supervisor) return false

      return true
    })
  }, [productionPlans, tableFilters])
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="space-y-1">
            <p className="text-sm font-light">{new Date(plan.start_date).toLocaleDateString('en-GB')}</p>
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="space-y-1">
            <p className="text-sm font-light">{new Date(plan.end_date).toLocaleDateString('en-GB')}</p>
      ...
      <AdminDashboardLayout title="Production Plans" subtitle="Manage and monitor production planning">
        <div className="space-y-6">
          ...
          <ProductionPlanFormDrawer
            ...
            productionPlan={editingPlan || undefined}

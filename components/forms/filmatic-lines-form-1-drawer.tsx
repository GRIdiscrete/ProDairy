...
export function FilmaticLinesForm1Drawer({
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && form) {
      ...
      let shiftType = ""
      const hasDayShift = !!form.day_shift_id
      const hasNightShift = !!form.night_shift_id

      if (hasDayShift) shiftType = "day_shift"
      else if (hasNightShift) shiftType = "night_shift"
      ...
      try {
        ...
        if (shiftType === "day_shift") {
          shiftEntry = form.day_shift_id
        } else if (shiftType === "night_shift") {
          shiftEntry = form.night_shift_id
        }

        if (shiftEntry) {
          const d = shiftEntry.shift_details
          const stoppage = d?.stoppage_time_id || {}

          const mappedDetail = {
            id: d?.id,
            time: d?.time?.split(' ')?.[1]?.slice(0, 5) || d?.time?.slice(0, 5) || '',
  ...
  const handleFinalSubmit = async (data: ShiftDetailsFormData) => {
    try {
      ...
      if (shiftType === "day_shift") {
        ...
        // Day shift object with single detail entry
        payload.day_shift = {
          ...(mode === "edit" && form?.day_shift_id?.id ? { id: form.day_shift_id.id } : {}),
          supervisor_approve: !!data.supervisor_approve,
          operator_id: data.operator_id || null,
          shift_details: {
            ...(firstDetail.id ? { id: firstDetail.id } : {}),
            time: firstDetail.time || null,
            pallets: firstDetail.pallets ?? null,
            target: firstDetail.target ?? null,
            setbacks: firstDetail.setbacks || null,
            stoppage_time_id: {
              ...((firstDetail.stoppage_time as any)?.id ? { id: (firstDetail.stoppage_time as any).id } : {}),
              product_1_hours: firstDetail.stoppage_time?.product_1_hours ?? null,
              product_2_hours: firstDetail.stoppage_time?.product_2_hours ?? null,
              filler_1_hours: firstDetail.stoppage_time?.filler_1_hours ?? null,
              filler_2_hours: firstDetail.stoppage_time?.filler_2_hours ?? null,
              product_1: firstDetail.stoppage_time?.product_1 ?? null,
              product_2: firstDetail.stoppage_time?.product_2 ?? null,
              filler_1: firstDetail.stoppage_time?.filler_1 ?? null,
              filler_2: firstDetail.stoppage_time?.filler_2 ?? null,
            }
          }
        }

        // Ensure night shift is null
        ...
        // Night shift object with single detail entry
        payload.night_shift = {

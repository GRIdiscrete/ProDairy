  ...
  <div className="mb-8 p-6 bg-blue-50 rounded-lg">
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm font-medium text-blue-600">Skimming</span>
...
const rawMilkSchema = yup.object({
  source_silo_name: yup.string().required("Source silo is required"),
  quantity: yup.number().transform(v => (v === "" || isNaN(v) ? undefined : v)).required("Quantity is required").min(0.1, "Quantity must be greater than 0"),
  fat: yup.number().transform(v => (v === "" || isNaN(v) ? undefined : v)).required("Fat content is required").min(0, "Fat content must be 0 or greater"),
})

const skimMilkSchema = yup.object({
  destination_silo_name: yup.string().nullable().optional(),
  quantity: yup.number().transform(v => (v === "" || isNaN(v) ? null : v)).nullable().optional(),
  fat: yup.number().transform(v => (v === "" || isNaN(v) ? null : v)).nullable().optional(),
})

const creamSchema = yup.object({
  cream_tank: yup.string().nullable().optional(),
  quantity: yup.number().transform(v => (v === "" || isNaN(v) ? null : v)).nullable().optional(),
  fat: yup.number().transform(v => (v === "" || isNaN(v) ? null : v)).nullable().optional(),
})
...
const skimmingFormSchema = yup.object({
  ...
  skim_milk: skimMilkSchema.nullable().optional(),
  cream: creamSchema.nullable().optional(),
})
...
export function SkimmingFormDrawer({
  ...
  const loadInitialData = async () => {
    try {
      ...
      const silosData = silosResponse.data?.map(silo => ({
        value: silo.name, // Store name now
      ...
      if (mode === "edit" && form) {
        formHook.reset({
          ...
          raw_milk: form.raw_milk ? {
            source_silo_name: form.raw_milk.source_silo_name || "",
            quantity: form.raw_milk.quantity || 0,
            fat: form.raw_milk.fat || 0,
          } : {
            source_silo_name: "",
            quantity: 0,
            fat: 0,
          ...
          skim_milk: form.skim_milk ? {
            destination_silo_name: form.skim_milk.destination_silo_name || "",
            quantity: form.skim_milk.quantity,
            fat: form.skim_milk.fat,
          } : {
            destination_silo_name: "",
            quantity: null,
            fat: null,
          ...
          cream: form.cream ? {
            cream_tank: form.cream.cream_tank || "",
            quantity: form.cream.quantity,
            fat: form.cream.fat,
          } : {
            cream_tank: "",
            quantity: null,
  ...
  const onInvalid = (errors: any) => {
    const errorMessages = Object.values(errors).map((err: any) => {
      if (typeof err === 'object' && err !== null && !err.message) {
        return Object.values(err).map((subErr: any) => subErr.message).join(', ')
      }
      return err.message
    }).filter(Boolean);
  ...
  const handleSubmit = async (data: SkimmingFormData) => {
    ...
    try {
      // Prepare payload - only include optional objects if they have data
      const hasSkimData = data.skim_milk?.destination_silo_name || data.skim_milk?.quantity != null || data.skim_milk?.fat != null
      const hasCreamData = data.cream?.cream_tank || data.cream?.quantity != null || data.cream?.fat != null

      const payload: any = {
        operator_id: user.id,
        operator_signature: normalizeDataUrlToBase64(data.operator_signature),
        raw_milk: {
          source_silo_name: data.raw_milk.source_silo_name,
          quantity: data.raw_milk.quantity,
          fat: data.raw_milk.fat,
          ...(mode === "edit" && form?.raw_milk?.id ? { id: form.raw_milk.id } : {})
        },
        skim_milk: hasSkimData ? {
          destination_silo_name: data.skim_milk?.destination_silo_name || null,
          quantity: data.skim_milk?.quantity || null,
          fat: data.skim_milk?.fat || null,
          ...(mode === "edit" && form?.skim_milk?.id ? { id: form.skim_milk.id } : {})
        } : null,
        cream: hasCreamData ? {
          cream_tank: data.cream?.cream_tank || null,
          quantity: data.cream?.quantity || null,
          fat: data.cream?.fat || null,
          ...(mode === "edit" && form?.cream?.id ? { id: form.cream.id } : {})
        } : null,
      }

      if (mode === "edit" && form) {
        payload.id = form.id
        payload.tag = form.tag // tag might be required for patch
        await dispatch(updateSkimmingForm(payload)).unwrap()
        toast.success("Skimming form updated successfully")
      } else {
        await dispatch(createSkimmingForm(payload)).unwrap()
        toast.success("Skimming form created successfully")
      }

      // refresh skimming list
            ...
            <form onSubmit={formHook.handleSubmit(handleSubmit, onInvalid)} className="space-y-6 p-6">
                ...
                <div className="border border-gray-200 rounded-lg bg-white border-l-4 border-l-green-500 p-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 col-span-2">
                      ...
                      <Controller
                        name="raw_milk.source_silo_name"
                        control={formHook.control}
                        render={({ field }) => (
                          <SearchableSelect
                            options={silos}
                      ...
                      />
                      {formHook.formState.errors.raw_milk?.source_silo_name && (
                        <p className="text-sm text-red-500">
                          {formHook.formState.errors.raw_milk.source_silo_name?.message}
                        </p>
                      )}
                    </div>
                ...
                <div className="border border-gray-200 rounded-lg bg-white border-l-4 border-l-[#006BC4] p-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 col-span-2">
                      ...
                      <Controller
                        name="skim_milk.destination_silo_name"
                        control={formHook.control}
                        render={({ field }) => (
                          <SearchableSelect
                            options={silos}
                      ...
                      />
                      {formHook.formState.errors.skim_milk?.destination_silo_name && (
                        <p className="text-sm text-red-500">
                          {formHook.formState.errors.skim_milk.destination_silo_name?.message}
                        </p>
                      )}
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div className="border border-gray-200 rounded-lg bg-white border-l-4 border-l-yellow-500 p-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2 col-span-2">
                      <Label htmlFor="cream_tank">Cream Tank</Label>
                      <Controller
                        name="cream.cream_tank"
                        control={formHook.control}
                        render={({ field }) => (
                          <SearchableSelect
                            options={silos.filter(s => s.description?.toLowerCase().includes("cream")).length > 0
                              ? silos.filter(s => s.description?.toLowerCase().includes("cream"))
                              : silos}
                            value={field.value || ""}
                            onValueChange={field.onChange}
                            placeholder="Select cream tank"
                            loading={loadingSilos}
                          />
                        )}
                      />
                      {formHook.formState.errors.cream?.cream_tank && (
                        <p className="text-sm text-red-500">
                          {formHook.formState.errors.cream.cream_tank?.message}
                        </p>
                      )}
                    </div>
